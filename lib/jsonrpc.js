...
jsonrpc.callMethod = function(req, route) {
    return function(results, cb) {
        ...
        route.handler[json.method](route.env, req, json.params,
            function(err, result) {
...
jsonrpc.middlewareIterator = function(req, resp, params, route) {
    return function(middleMethod, cb) {
        log.debug("At least one iterator !", middleMethod);
        middleMethod(req, resp, params, route, function(err, results) {
            cb(err, results);
        });
    };
};
jsonrpc.runMiddleware = function(when, req, resp, route) {
    log.debug("ROUTE:", route);
    log.debug("WHEN:", when);
    log.debug("REQ:", typeof(req));
    log.debug("RESP:", typeof(resp));
        ...
        */
        ...
        req.on('end', function() {
            async.auto({
                ...
                runPreMiddleware: ['parse', 'validateParams', jsonrpc.runMiddleware('before', req, resp, route)],
                callMethod: ['parse', 'runPreMiddleware', 'validateParams', jsonrpc.callMethod(req, route)],
                runPostMiddleware: ['parse', 'callMethod', jsonrpc.runMiddleware('after', req, resp, route)]
